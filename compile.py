...
from exprtree import compile_tree, Print

class Program:
    def __init__(self):
        self.first_stmt = -1        # 'main' not yet found
        self.last_temp  = 0         # no registers yet claimed
        self.functions = BinaryTree()  # no functions yet
        self.variables = BinaryTree()  # no global variables yet
...
def compile_line(program,expr):
    ...
    if peek(iterator) == "deffn":
        ...
        compile_tree( body, local_variables, program.functions, program.code )  # program code - output
    else:
